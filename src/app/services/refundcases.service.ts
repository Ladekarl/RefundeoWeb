...
export class RefundCasesService {
  ...
  private static mapDate(refundCase: RefundCase): RefundCase {
    if (refundCase) {
      refundCase.dateCreated = (new Date(refundCase.dateCreated)).toLocaleDateString();
      refundCase.dateRequested = (new Date(refundCase.dateRequested)).toLocaleDateString();
    }
    return refundCase;
  }
  ...
  getAll() {
    return this.http.get<RefundCase[]>('/api/merchant/refundcase')
      .map(RefundCasesService.mapDates);
  }

  getPaginated(first: number, amount: number, sortBy: string, sortDir: number) {
    return this.http.get(`/api/merchant/refundcase/${first}/${amount}/${sortBy}/${sortDir}`)
      .map((r: any) => {
